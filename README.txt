...
============
	...
	This package contains Redpine open source driver (rsi) package for both
	RS9113 and RS9116 chip sets. This file gives the instruction to build
	and install the driver. For instructions on complete usage of the 
	driver and module, please refer RSI_OpenSource_Driver_TRM_vX.X.pdf in
	documents folder.
...
========
	* insmod rsi_91x.ko rsi_zone_enabled=0x1f dev_oper_mode=1
	...
	bit 10 - mgmt debug zone
      ...
      $ insmod rsi_91x.ko rsi_zone_enabled=0x3 dev_oper_mode=<val1>,<val2>,<val3>,<val4>,<val5>
      	
       * Max 5 modules we support.
       * Multimode is not supported in Zigbee.
	...
	* insmod rsi_sdio.ko
      ...
      While using SDIO interface, To change SDIO clock, User can give their board specific
      SDIO clock using below command
      ...
      for ARM32, Keep sdio_clock between 12 to 20.

      To enable antenna diversity feature.(only in sta mode)
      
	$ insmod rsi_91x.ko rsi_zone_enabled=0x1f dev_oper_mode=1 antenna_diversity=<val>
      where,
      val = 0 (Disable Antenna Diversity)
	    1 (Enable Antenna Diversity)
      By defualt Antenna Diversity is disable.
...
===============================
To get driver and firmware version use below commands
	$ cat /sys/kernel/debug/phy<x>/version

	where phy<x> is hardware device number that we can get by using below commands
	$iw dev
